package com.pain.log.painlog.log;
...
public class ChangeLog {
	...
	"body {                                                           " + "	font-family: Verdana, Helvetica, Arial, sans-serif;   " + "	font-size: 11px;                                      " + "	color: #000000;                                       " + "	background-color: #eeeeee;                            " + "	margin: 0px;                                          " + "	padding: 0px;                                         " + "}                                                        "
	 ...
	 * 
	 * <p>
	 * Future calls to {@link #isFirstRun()} and {@link #isFirstRunEver()} will
	 * return {@code false} for the current app version.
	 * </p>
	 */
	...
	protected AlertDialog getDialog(boolean full) {
		...
		wv.loadData(getLog(full), "text/html; charset=UTF-8", null);
		...
		// OK button
				.setPositiveButton(mContext.getResources().getString(R.string.changelog_ok_button), new DialogInterface.OnClickListener() {
					@Override
					public void onClick(DialogInterface dialog, int which) {
						// The user clicked "OK" so save the current version
						// code as
						// "last version code".
						updateVersionInPreferences();
					}
		...
		if (!full) {
			// Show "More…" button if we're only displaying a partial change
			// log.
	 ...
	 *            version are returned.
	 * 
	 * @return A sorted {@code List} containing {@link ReleaseItem}s
	 *         representing the (partial) change log.
	 * 
	 ...
	 *            version are returned.
	 * 
	 * @return A {@code SparseArray} containing {@link ReleaseItem}s
	 *         representing the (partial) change log.
	 */
	...
	private boolean parseReleaseTag(XmlPullParser xml, boolean full, SparseArray<ReleaseItem> changelog) throws XmlPullParserException, IOException {
		...
		String date = xml.getAttributeValue(null, ReleaseTag.DATE);
		...
		ReleaseItem release = new ReleaseItem(versionCode, version, changes, date);
	...
	/**
	 * Returns a {@link Comparator} that specifies the sort order of the
	 * {@link ReleaseItem}s.
	 * 
	 * <p>
	 * The default implementation returns the items in reverse order (latest
	 * version first).
	 * </p>
	 */
	...
	public static class ReleaseItem {
		...
		ReleaseItem(int versionCode, String versionName, List<String> changes, String date) {
